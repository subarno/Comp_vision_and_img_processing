...
ArrayList<String> buildSymbolSequence(ArrayList<ArrayList<PVector>> strokes) {
  ...
  for (ArrayList<PVector> stroke : strokes) {
    ...
    // Build this stroke's symbol string
    String str = "";
    for (int i = 1; i < N - 1; i++) {
      float xm1 = xs[i-1], x0 = xs[i], x1 = xs[i+1];
      float ym1 = ys[i-1], y0 = ys[i], y1 = ys[i+1];
      // local X‐min
      if (x0 < xm1 && x0 < x1) str += "A";
      // local X‐max
      if (x0 > xm1 && x0 > x1) str += "B";
      // local Y‐min
      if (y0 < ym1 && y0 < y1) str += "C";
      // local Y‐max
      if (y0 > ym1 && y0 > y1) str += "D";
    }
    seq.add(str);
  }
  return seq;
